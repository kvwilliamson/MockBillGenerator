...
function App() {
    ...
    const [v3ChargeRate, setV3ChargeRate] = useStickyState('FMV', 'MB_V3_ChargeRate'); // FMV | CHARGEMASTER
            ...
            <div className="w-1/3 bg-white p-6 rounded-xl shadow-lg h-fit">
                ...
                {activeTab === 'V3' && (
                    <div className="space-y-6">
                        ...
                        <div>
                            <label className={`block text-sm font-medium mb-1 ${v3PayerType === 'Self-Pay' ? 'text-slate-700' : 'text-slate-400'}`}>
                                Charge Rate Selection
                            </label>
                            <select
                                value={v3ChargeRate}
                                onChange={(e) => setV3ChargeRate(e.target.value)}
                                disabled={v3PayerType !== 'Self-Pay'}
                                className={`w-full p-2.5 border rounded-md text-sm shadow-sm transition ${v3PayerType === 'Self-Pay'
                                    ? 'bg-white border-slate-300 text-slate-900 cursor-pointer'
                                    : 'bg-slate-50 border-slate-200 text-slate-400 cursor-not-allowed'
                                    }`}
                            >
                                <option value="CHARGEMASTER">Chargemaster (High Inflation)</option>
                                <option value="FMV">Fair Market Value (Market Rate)</option>
                            </select>
                        </div>
                                ...
                                <details className="group">
                                    ...
                                    <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        {generatedData.simulation_debug?.sentinel_truth && (
                                            <details className="group bg-black/50 rounded border border-red-900/30 shadow-inner open:bg-black/60 transition-all">
                                                <summary className="p-3 cursor-pointer list-none flex items-center justify-between text-red-400 text-[10px] font-bold uppercase tracking-tighter hover:text-red-300">
                                                    <span>Phase 6: Compliance Sentinel (Enforcement)</span>
                                                    <span className="group-open:rotate-90 transition-transform">▶</span>
                                                </summary>
                                                <pre className="px-3 pb-3 text-[10px] text-slate-300 overflow-x-auto max-h-[300px] overflow-y-auto border-t border-red-900/30 pt-2 mt-1">
